insert into items
select replace(name, 'Light Armor', 'Buckler Shield') as name, replace(description, 'light armor', 'a buckler shield') as description, cost as cost, 'Shield - Buckler - Mod' as type,
		'Jewelcrafting - Gems' as subtype
from items
where subtype = 'Jewelcrafting - Gems'
  and type = 'Armor - Light - Mod'


insert into recipies
select newResult.id, resultQty, profession, crafterLevel, 2 as [hours]
...
from recipies oldRecipie
	 ...
	 inner join items newResult on newResult.name = replace(oldResult.name, 'Light Armor', 'Buckler Shield')
where newResult.id != oldResult.id
  and oldResult.subtype = 'Jewelcrafting - Gems'




select * from recipies where result in (select id from items where name like '%Heavy Shield' and subtype = 'Jewelcrafting')
...
where oldResult.subtype = 'Jewelcrafting - Gems'
  and newResult.id != oldResult.id
  and recipieIngredients.ingredient = newIngredient.id
  and (((newRecipie.id % 3 = 0) and (newIngredient.name like '%gem'))
       or ((newRecipie.id % 3 = 1) and (newIngredient.name like '%shard'))
	   or ((newRecipie.id % 3 = 2) and (newIngredient.name like '%peb%')))
